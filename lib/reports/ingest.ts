...
export interface Store {
  ...
  deleteRange(table: string, start: string, end: string): Promise<number>;
...
export interface StagedSummary {
  ...
  dateEnd: string | null;
  perType: { reportType: ReportType; label: string; filename: string; rows: number }[];
  totalRows: number;
}
...
export function stage(reports: ParsedReport[]): StagedSummary {
  ...
  const hasCampaignReport = reports.some((r) => r.reportType === TOTALS_SOURCE);
  if (!hasCampaignReport && reports.length > 0) {
    errors.push(
      "No Campaign report in this batch. Headline totals come only from the Campaign report, so add it before saving."
    );
  }
...
export async function commit(
  ...
  for (const r of reports) {
    ...
    deleted += await store.deleteRange(table, r.dateStart, r.dateEnd);
